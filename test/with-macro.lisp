  ...
  (handler-bind ((style-warning #'muffle-warning))
    (macrolet ((frob (&body body)
      ...
      (finishes (frob
                 ...
                 (def (with-macro* :quoted-arguments lexical-var-name) with-foo4 (lexical-var-name foo &key ((:bar bar-local-name)))
                   (bind ((local-var (* 2 foo))
                          (*with-foo/special* (+ local-var bar-local-name)))
                     (-with-macro/body- (local-var lexical-var-name))))
                 (def (with-macro* :quoted-arguments (new-var-name)) with-foo5 (new-var-name foo &rest args &key ((:bar bar-local-name)) (keyword-defaulting (+ 2 2)) &allow-other-keys)
                   (bind ((local (* 2 foo))
